...
-(PGBRealmBook *)helperMethodToValidateBooks:(Book *)book
{
    ...
    realmBook.ebookID = book.eBookNumbers;
    
    if ([book.eBookNumbers isEqualToString:@""]) {
        return nil;
    }
    
    realmBook.author = book.eBookAuthors;
    if ([book.eBookAuthors isEqualToString:@""])
    {
        //if the author information is missing, check to see if friendly title information is present
        if (![book.eBookFriendlyTitles isEqualToString:@""])
        {
            ...
            NSArray *stringToArray = [book.eBookFriendlyTitles componentsSeparatedByString:@" "];
        ...
        else if ([book.eBookAuthors isEqualToString:@""]) {
            realmBook.author = @"";
        }
    }
